...
class FlexToFListener(Node):
    def __init__(self, calibrate=False):
        ...
        # --- NEW: enabled flag; start idle until service call ---
        self.enabled = False

        # State machine: start in approach but controller is idle until enabled
        self.state = 'approach'
        ...
        # Start in 'default' logic but we will not run it until enabled
        self.controller = 'default'

        # NOTE: We do NOT call _setup_servo_clients() or _enable_servo_mode() here.
        # Those calls are deferred until the /eva_controller_relative_motion service is called.

        # Create a service that start_harvest can call to enable relative motion behavior
        # This service will initialize servo clients and enable operation.
        self._start_srv = self.create_service(Trigger, '/eva_controller_relative_motion', self._start_service_callback)

        self.get_logger().info('FlexToFListener initialized and idling (waiting for /eva_controller_relative_motion).')

    # --- Service callback to activate controller ---
    def _start_service_callback(self, request, response):
        try:
            self.get_logger().info("Received request to start relative motion controller...")
            self._setup_servo_clients()
            self._enable_servo_mode(frame="tool0")
            self.enabled = True
            self.get_logger().info("Relative motion controller enabled.")
            response.success = True
            response.message = "Controller enabled"
        except Exception as e:
            self.get_logger().error(f"Failed to enable controller: {e}")
    ...
    def control_loop(self):
        # If not enabled, do not run controller logic or publish commands.
        if not self.enabled:
            # Optionally, you can still publish zeros or status; keep quiet for safety.
            return
        ...
        if self.controller == 'default':
            if self.state == 'servo':
                ...
                if self.tof_distance > self.tof_servo_threshold and (ex > self.position_threshold or ey > self.position_threshold):
                    ...
                    self.controller = 'eva_controller_relative_motion'
        if self.controller == 'eva_controller_relative_motion':
            if self.state == 'servo':
                if (ex < self.position_threshold and ey < self.position_threshold) or self.tof_distance <= self.tof_servo_threshold:
                    self.state = 'approach'
            if self.state == 'approach':
                if ex > self.position_threshold or ey > self.position_threshold:
                    self.state = 'servo'
                if self.get_tof_diff() is not None and self.get_tof_diff() < 0: # apple is getting closer
                    print("apple is getting closer")
                elif self.get_tof_diff() is not None and self.get_tof_diff() > 1: # apple is being pushed away
            ...
            if self.state == 'reverse':
                if self.get_tof_diff() is not None and self.get_tof_diff() < 0:
                    print("apple is getting closer")
                elif self.get_tof_diff() is not None and self.get_tof_diff() > 1:
    ...
    def _setup_servo_clients(self):
        """
        Setup service clients. This may block until those services are available;
        we intentionally do this during activation rather than at node startup.
        """
    ...
    def _enable_servo_mode(self, frame: str = "tool0"):
        """
        Switch controller & start MoveIt-Servo. This is safe to call from the activation
        service since we waited for the clients in _setup_servo_clients.
        """
